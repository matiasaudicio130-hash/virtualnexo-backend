...
def send_kyc_approved_email(to_email: str, first_name: str) -> None:
    content = (
        _h2(f"Identidad verificada, {first_name}")
...
def send_kyc_rejected_email(to_email: str, first_name: str, reason: str = "") -> None:
    motivo = f'<p style="color:#EF4444;font-size:13px;margin:0 0 14px;">Motivo: {reason}</p>' if reason else ""
    content = (
        _h2(f"Verificación no exitosa")
        + _p(f"Hola {first_name}, no pudimos verificar tu identidad.")
        + motivo
...
def send_manual_approval_email(to_email: str, first_name: str) -> None:
    content = (
        ...
        + _p(f"Hola {first_name}, recibimos tu solicitud de acceso a AURA.")
