...
STEALTH_MARKER  = b"\xDE\xAD\xBE\xEF"  # Firma para detectar watermark
...
def _wm_label(user_id: str) -> str:
    """Genera la etiqueta visible: 'VNX·XXXXXX' (últimos 6 del ID)."""
    ...
    def verify_leak(self, image_data: bytes) -> dict:
        ...
        try:
            ...
            if not payload:
                return {"found": False}
            parts = payload.split(":")
            if len(parts) >= 4 and parts[0] == "VNX":
                return {
                    "found": True,
                    "user_id": parts[1],
                    "timestamp": parts[2],
