...
import { Chat, Heart, DotsThree, ShareNetwork, ShieldSlash, Flag } from "@phosphor-icons/react";
  ...
  return (
    <div style={{ display: "flex", gap: 10, justifyContent: "center", marginBottom: 20 }}>
      ...
      <div style={{ position: "relative" }}>
        ...
        >
          <DotsThree size={15} strokeWidth={1.5}/>
        </button>
        {menuOpen && (
          <>
            ...
            >
              <MenuItem icon={ShareNetwork}    label="Compartir" onClick={() => { setMenuOpen(false); onShare(); }} />
              <MenuItem icon={ShieldSlash} label="Bloquear"  onClick={() => { setMenuOpen(false); onBlock(); }} />
...
function MenuItem({ icon: Icon, label, onClick, danger }: { icon: Icon; label: string; onClick: () => void; danger?: boolean }) {
  return (
    <button
      onClick={onClick}
      style={{
